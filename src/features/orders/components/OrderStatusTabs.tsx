...
const OrderStatusTabs = ({ orders, onStatusChange }: OrderStatusTabsProps) => {
  ...
  const deliveredOrders = allOrders.filter(order => order.status === 'delivered');
    ...
    const renderOrdersTable = (filteredOrders: AdminOrder[], emptyMessage: string, tabKey: string) => {
    ...
    // Add these lines:
    const currentPage = currentPages[tabKey as keyof typeof currentPages];
    ...
    return (
                  ...
                  {order.user_id ? (
                    ...
                    <>
                      <p className="font-medium text-gray-900 dark:text-white">
                        {order.customer_detail?.customer_name || 'Unknown Customer'}
                      </p>
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        {order.customer_detail?.shipping_address?.phone}
                      </p>
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        {order.customer_detail?.shipping_address?.address}
                      </p>
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        {order.customer_detail?.shipping_address?.city}, {order.customer_detail?.shipping_address?.state} {order.customer_detail?.shipping_address?.zipCode}
                      </p>
                      {order.customer_detail?.shipping_address?.latitude && order.customer_detail?.shipping_address?.longitude && (
                        <div className="mt-2">
                          <a 
                            href={`https://maps.google.com/?q=${order.customer_detail.shipping_address.latitude},${order.customer_detail.shipping_address.longitude}`}
                            target="_blank"
      ...
      <Pagination
        ...
        onPageChange={(page) => setCurrentPages(prev => ({ ...prev, [tabKey]: page }))}
      ...
      <CardContent>
       <Tabs defaultValue="all" className="w-full" onValueChange={(value) => setActiveTab(value)}>
          ...
          <TabsContent value="all" className="mt-6">
            {renderOrdersTable(allOrders, "No orders found.", "all")}
          </TabsContent>

          <TabsContent value="pending" className="mt-6">
            {renderOrdersTable(pendingOrders, "No pending orders found.", "pending")}
          </TabsContent>

          <TabsContent value="processing" className="mt-6">
            {renderOrdersTable(processingOrders, "No processing orders found.", "processing")}
          </TabsContent>

          <TabsContent value="delivered" className="mt-6">
            {renderOrdersTable(deliveredOrders, "No delivered orders found.", "delivered")}
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>
